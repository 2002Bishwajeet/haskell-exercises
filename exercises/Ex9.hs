...
data StackProg a where
  Done :: StackProg a
  PushInt :: Int -> StackProg a -> StackProg (Int, a)
  AddTop :: StackProg (Int, (Int, a)) -> StackProg (Int, a)

prog :: StackProg (Int, ())
prog = AddTop (PushInt 2 (PushInt 1 Done))

data ListStatus = Empty | NonEmpty

data SafeList s a where
  Nil :: SafeList 'Empty a
  Cons :: a -> SafeList s a -> SafeList 'NonEmpty a

safeHead :: SafeList 'NonEmpty a -> a
safeHead (Cons x _) = x

safeLast :: SafeList 'NonEmpty a -> a
safeLast (Cons x xs) = case xs of
  Nil -> x
  Cons _ _ -> safeLast xs

type family AppendStatus (s1 :: ListStatus) (s2 :: ListStatus) :: ListStatus where
  AppendStatus 'NonEmpty s2 = 'NonEmpty
  AppendStatus 'Empty s2 = s2

safeAppend :: SafeList s1 a -> SafeList s2 a -> SafeList (AppendStatus s1 s2) a
safeAppend Nil ys = ys
safeAppend (Cons x xs) ys = Cons x (safeAppend xs ys)

showSafeList :: (Show a) => SafeList s a -> String
showSafeList Nil = ""
showSafeList (Cons x xs) = case xs of
  Nil -> show x
  Cons _ _ -> show x ++ " " ++ showSafeList xs

toList :: SafeList s a -> [a]
toList Nil = []
-- toList (Cons x xs) = case xs of
--   Nil -> [x]
--   Cons _ _ -> x : toList xs
